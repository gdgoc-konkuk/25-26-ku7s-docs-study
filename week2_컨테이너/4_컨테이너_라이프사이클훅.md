...
정의: 쿠버네티스에서 컨테이너는 **“생성 → 실행 → 종료”** 라는 **생명주기(lifecycle)**를 가지는데, 이 과정 중에 **특정 시점(PostStart/PreStop)**에 사용자 정의 동작(스크립트나 명령)을 실행할 수 있게 해주는 기능이다.

* **특정 시점**: 컨테이너 이미지 + 환경 + 런타임 등이 준비된 이후, 혹은 종료되기 직전
...
| **StopSignal** *(v1.33 추가)* | 컨테이너가 종료될 때 전달할 시그널을 지정 | SIGTERM 외 다른 시그널을 전달해야 할 때 사용(ex. SIGINT) |
...
- `PreStop`은 종료 신호(SIGTERM 등)를 받기 직전에 실행되며, 이 훅이 끝날 때까지 컨테이너는 실제로 종료되지 않는다.
...
| **httpGet** | 컨테이너 내부 or 외부에서  HTTP 요청을 해당 컨테이너의 엔드포인트로 전송 | `path: /ready`, `port: 8080` |
...
* grace period: 쿠버네티스가 컨테이너를 바로 죽이지 않고 **정상적으로 종료할 수 있게 기다려주는 시간**
