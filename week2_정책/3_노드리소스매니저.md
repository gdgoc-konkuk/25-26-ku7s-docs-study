...
- Exclusive allocation(독점 할당): 특정 pod가 CPU 코어 몇개를 독점적으로 사용
- 왜?
...
- 프로세스
  ...
  3. BestEffort/Burstable 파드, Guaranteed 파드 중에서 정수가 아닌 CPU 요청 → 공유 풀에서 배정
  4. Guaranteed pod & 정수 요청 CPU → 독점 배정
  ...
  spec:
    containers:
      - name: nginx
        image: nginx
  ```
  이러한 pod는 request와 limits가 정의되어 있지 않기 때문에 BestEffort이고 shared pool에서 동작
  ```yaml
  spec:
    containers:
      - name: nginx
