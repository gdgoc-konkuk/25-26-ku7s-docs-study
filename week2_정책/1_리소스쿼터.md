...
3. 한도 초과 시 거부
   ...
   API 서버가 HTTP 4033 Forbidden과 함께 에러 메시지를 반환
...
지정된 네임스펭스에서 요청할 수 있는 총 스토리지 리소스 합을 제안하는 기능
...
> 코어 그룹: 가장 기본적인 리소스들
>
> Pod, Service, ConfigMap, Secret, PVC, Namespace, Node emd
>
...
네임스페이스를 넘어서 파드 간 affinity를 설정하지 못하도록 제안하는 정책
        ...
        limitedResources:
          - resource: pods
            matchScopes:
              - scopeName: CrossNamespaceAffinity
  ```
  → CrossNamepsaceAffinity를 사용하는 파드의 경우 반드시 ResourceQuota로 허용된 네임스페이스에서만 만들 수 있음

## Request와 Limit

네임스페이스에 쿼터가 정의되어있으면 그 네임스페이스 안에서 만드는 모든 파드는 반드시 request 값을 적어야 함 → 네임스페이스 전체의 자원 사용 총량을 제안하기위한 쿼터 계산시 필요
...
- 클러스터에 노드를 더 추가해도 네임스페이스의 쿼터는 자동적으로 제한됨
...
PriorityClass를 cluster-service(가장 높은 우선순위)를 쓰게되면 절대 종료되지 않는 파드가 새김 → cluster-sevice는 kube-system 네임스페이스 안에서만 쓰도록 제한
